...
def search(request):
    ...
    breadcrumbs = [{'name': 'Home', 'url': reverse('home_page')}, {'name': 'Search Results', 'url': f"?q={query}"}]

    if product_slug:
        product = get_object_or_404(Product, slug=product_slug)
        breadcrumbs.append({'name': product.name, 'url': reverse('product_detail', kwargs={'slug': product.slug})})
